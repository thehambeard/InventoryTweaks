...
namespace InventoryTweaks.UI
{
    class ContainerTypeUIManager : MonoBehaviour
    {
        ...
        private void UpdateContainers()
        {
            ...
            int newCount = 0;
            bool isDirty = false;
            List<HiButtonManager> scrolls = new List<HiButtonManager>();
            foreach (var item in Game.Instance.Player.Inventory
                .Where(c => (c.Blueprint as BlueprintItemEquipmentUsable != null) && (c.Blueprint as BlueprintItemEquipmentUsable).Type == UseableType)
                .OrderBy(item => item.Name))
            {
                scrolls.Add(EnsureContainer(item, newCount++, ref isDirty));
            }
